...
const VipTab: React.FC<VipTabProps> = ({ onNavigate }) => {
  ...
  // Handler for initiating purchase, tailored for Facebook Payment integration
  const handlePurchase = (price: number, level: number) => {
    // NOTE: In a live Facebook application, this function would typically use 
    // the Facebook SDK (FB.ui({ method: 'pay', ... })) to open the payment dialog.
    
    // Placeholder implementation simulating the action:
    console.log(`Attempting Facebook Payment for VIP Level ${level} ($${price}).`);
    
    // We navigate to a hypothetical payment processing tab or show a confirmation.
    // Since we are simulating a Facebook app, we assume the payment flow is external/handled by FB.
    alert(`سيتم فتح شاشة دفع فيسبوك لترقية باقة VIP ${level} بسعر $${price}.`);
    
    // Optional: Navigate away (e.g., to the dashboard after purchase initiation)
    // onNavigate('home'); 
  };
  ...
  return (
    <div className="space-y-6 animate-scale-in">
      ...
      <div className="text-center mb-8">
        {/* Adjusted wording slightly to emphasize platform context */}
        ...
        <h2 className="text-3xl font-black text-gradient-gold">باقات VIP (تطبيق فيسبوك)</h2>
        <p className="text-muted-foreground mt-2">اختر باقتك وابدأ بجني الأرباح عبر منصة فيسبوك</p>
      </div>
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-between items-center">
                ...
                <Button 
                  onClick={() => handlePurchase(price, level)} // Changed action to FB Purchase handler
                  variant="gold"
                  className="gap-2"
                >
                  <Sparkles size={16} />
